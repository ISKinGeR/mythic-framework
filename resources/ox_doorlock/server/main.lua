...
local function isAuthorised(playerId, door, lockpick)
    ...
    local authorised = door.passcode or false
    local isHouse = door.name:find("@")
    local isCheckComplete = false

    if lockpick then return DoesPlayerHaveItem(player, Config.LockpickItems) end
    if door.characters and table.contains(door.characters, GetCharacterId(player)) then return true end
    if door.groups then authorised = IsPlayerInGroup(player, door.groups) and true or nil end
    if not authorised and door.items then authorised = DoesPlayerHaveItem(player, door.items) or nil end
    if not authorised and door.passcode then authorised = door.passcode == lib.callback.await('ox_doorlock:inputPassCode', playerId) end
	if not authorised and isHouse then
		local doorRealName = door.name:sub(1, door.name:find("@") - 1)
		local SID = Fetch:Source(playerId):GetData("Character"):GetData("SID")
		authorised = exports['mythic-ahs']:CheckPermForHouse(SID, doorRealName)
	end
    return authorised
end
